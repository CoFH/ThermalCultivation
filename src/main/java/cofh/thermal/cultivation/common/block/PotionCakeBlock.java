...
public class PotionCakeBlock extends CakeBlockCoFH implements EntityBlock {
    ...
    public BlockEntity newBlockEntity(BlockPos pos, BlockState state) {
        ...
        return new PotionCakeBlockEntity(pos, state);
    }
    ...
    public void setPlacedBy(Level worldIn, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack stack) {
        ...
        if (tile instanceof PotionCakeBlockEntity) {
            ((PotionCakeBlockEntity) tile).cacheEffects(stack.getTag());
        }
    }
    ...
    public ItemStack getCloneItemStack(BlockGetter worldIn, BlockPos pos, BlockState state) {
        ...
        BlockEntity tile = worldIn.getBlockEntity(pos);
        if (tile instanceof PotionCakeBlockEntity) {
            ((PotionCakeBlockEntity) tile).createItemStackTag(stack);
        }
        return stack;
    }
    ...
    protected InteractionResult eatPiece(Level world, BlockPos pos, BlockState state, Player player) {
        ...
        if (!player.canEat(true)) {
            ...
            if (!world.isClientSide && tile instanceof PotionCakeBlockEntity) {
                ((PotionCakeBlockEntity) tile).applyEffects(player);
            }
